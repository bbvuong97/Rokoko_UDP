...
class LeapPybulletIK():
    def __init__(self):
        ...
        p.setAdditionalSearchPath("/leap_hand_mesh")
        ...
        # TODO: what is the unit here?? seems dm, need to change this to adpat to fingercap
        self.fingertip_offset = np.array([0.1, 0., -0.06]) # VisionFT Since the root of the fingertip mesh in URDF is not at the tip (it is at the right lower part of the fingertip mesh), we set an offset to correct the fingertip location
        self.thumb_offset = np.array([0.1, 0., -0.06]) # VisionFT Same reason for the thumb tip
        # self.fingertip_offset = np.array([0.1, 0., -0.08]) # Since the root of the fingertip mesh in URDF is not at the tip (it is at the right lower part of the fingertip mesh), we set an offset to correct the fingertip location
        # self.thumb_offset = np.array([0.1, 0., -0.06]) # Same reason for the thumb tip
    ...
    def get_joint_limits(self, robot):
        ...
        # print(joint_lower_limits, joint_upper_limits, joint_ranges)
        # exit()
    ...
    def calculate_joint_poses(self, leap_id, end_effector_indices, end_effector_positions, hand_params):
        joint_poses = []
        with ThreadPoolExecutor() as executor:
            futures = []
            for i in range(4):
                future = executor.submit(p.calculateInverseKinematics, leap_id, end_effector_indices[i], end_effector_positions[i],
                                        lowerLimits=hand_params['lower_limits'], upperLimits=hand_params['upper_limits'], jointRanges=hand_params['joint_ranges'],
                                        restPoses=hand_params['rest_poses'], maxNumIterations=500, residualThreshold=0.005)
                futures.append(future)
            
            for i, future in enumerate(futures):
                joint_poses += list(future.result())[4 * i:4 * (i + 1)]
    ...
    def compute_IK(self, right_hand_pos, right_hand_wrist_ori, left_hand_pos, left_hand_wrist_ori):
        ...
        hand_params = {
        'lower_limits': self.hand_lower_limits,
        'upper_limits': self.hand_upper_limits,
        'joint_ranges': self.hand_joint_ranges,
        'rest_poses': self.HAND_Q.tolist()
        }

        # jointPoses_2 = self.calculate_joint_poses(self.LeapId_2, self.leapEndEffectorIndex, leapEndEffectorPos_2, hand_params)
        jointPoses = self.calculate_joint_poses(self.LeapId, self.leapEndEffectorIndex, leapEndEffectorPos, hand_params)
